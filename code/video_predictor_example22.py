...
def process_batch(batch_number):
    ...
    rendered_frames_dir_list = ['./rendered_frames', './rendered_frames_road', './rendered_frames_lane',
                                './rendered_frames_feedback']
    for i in rendered_frames_dir_list:
        if i == './rendered_frames_road':
    ...
    for ann_obj_id in (set(abs(labels_np))):
        ...
        for out_frame_idx in range(len(frame_filenames)):
            ...
            cv2.imwrite(
                os.path.join(rendered_frames_dir_list[ann_obj_id], f"road{videoNumber}_{present_count:05d}.png"),
                color_mask_image)
            present_count = present_count + 1
    image_counter = present_count
...
if os.path.exists(f"points_labels_video{videoNumber}.json"):
    load_user_points()
else:
    import threading
