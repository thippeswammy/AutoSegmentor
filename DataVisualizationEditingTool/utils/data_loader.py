...
class DataLoader:
    ...
    def load_data(self):
        ...
        `self.directory` and processes each file to extract points. It constructs nodes
        and edges while performing data integrity checks, such as ensuring the presence
        of at least two columns in the data. The function maintains a unique point ID
        for each node and calculates the maximum distance between points if valid data
        is loaded.
