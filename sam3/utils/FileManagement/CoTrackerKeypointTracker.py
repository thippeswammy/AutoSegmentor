...
class CoTrackerKeypointTracker:
    ...
    def __init__(self, keypoint_defs, initial_coords, frame_paths, checkpoint, window_len=60, query_frame_idx=0):
        ...
        # Convert to the same output format as KeypointTracker
        T = tracks_np.shape[0]
        for t in range(T):
            frame_kps = []
...
def track_between_frames(keypoint_coords, frame_from_path, frame_to_path, checkpoint, window_len=60):
    ...
    # Resize to match if needed (just stack as 2-frame video)
    ...
    # Return positions at frame 1 (the destination frame)
    result = []
    for i, coord in enumerate(sorted_coords):
